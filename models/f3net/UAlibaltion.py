...
class LKAUChangeST(nn.Layer):
    def __init__(self,in_channels=6, num_classes=2, fkernels=7):
        super().__init__()
        self.in_channels = in_channels
        self.encode1 = backbone(3)#(3, 34)
        self.encode2 = backbone(3)

        self.lkff1 = LKFF(64,fkernels)
        self.lkff2 = LKFF(128,fkernels)
        self.lkff3 = LKFF(256,fkernels)
        self.lkff4 = LKFF(512,fkernels)
        ...
        self.up1 = LSFFUp(1024, 256, fkernels)
        self.up2 = LSFFUp(512, 128, fkernels)
        # self.up3 = LSFFUp(256, 64, fkernels)
        # self.up4 = LSFFUp(128, 64, fkernels)
...
class LKAUChange_noPPM(nn.Layer):
    def __init__(self,in_channels=6, num_classes=2, fkernels=7):
        super().__init__()
        self.in_channels = in_channels
        self.encode1 = backbone(3)#(3, 34)
        ...
        self.ppm = layers.ConvBNPReLU(512,256,3)#layers.attention.PAM(512)
    ...
    def forward(self, x):
        ...
        # self.flast = self.ppm(self.augf4)
        y = self.ppm(self.augf4)
        # y = self.up1(self.flast, self.augf4)
        # print(y.shape, self.augf3.shape)
        y = self.up2(y, self.augf3)
        y = self.up3(y, self.augf2)
        y = self.up4(y, self.augf1)
        # y = nn.functional.interpolate(y, scale_factor=2, mode='bilinear', align_corners=True)
