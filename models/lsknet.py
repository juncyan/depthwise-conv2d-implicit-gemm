...
class LSKblock(nn.Layer):
    ...
    def forward(self, x):
        ...
        attn = paddle.cat([attn1, attn2], dim=1)
        avg_attn = paddle.mean(attn, dim=1, keepdim=True)
        max_attn, _ = paddle.max(attn, dim=1, keepdim=True)
        agg = paddle.cat([avg_attn, max_attn], dim=1)
...
class LSKNet(nn.Layer):
    ...
    def init_weights(self):
        ...
        #             trunc_normal_init(m, std=.02, bias_attr=0.)
        #         elif isinstance(m, nn.LayerNorm):
        #             constant_init(m, val=1.0, bias_attr=0.)
        #         elif isinstance(m, nn.Conv2D):
        #             fan_out = m.kernel_size[0] * m.kernel_size[
        #                 1] * m.out_channels
        #             fan_out //= m.groups
        #             normal_init(
        #                 m, mean=0, std=math.sqrt(2.0 / fan_out), bias_attr=0)
        # else:
