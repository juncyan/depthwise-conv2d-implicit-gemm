...
class FFN(nn.Layer):
    def __init__(self, in_channels, internal_channels, out_channels):
        super().__init__()
        self.cbr1 = layers.ConvBNReLU(in_channels, internal_channels, 3)
        self.cbr2 = layers.ConvBNReLU(internal_channels, out_channels, 3)
        self.lastbn = nn.BatchNorm2D(out_channels)
        self.relu = nn.ReLU()
...
class LRFE(nn.Layer):
    #Large receptive field ectraction
    def __init__(self, in_channels, dw_channels, block_lk_size, stride=1):
        super().__init__()
        self.cbr1 = layers.ConvBNReLU(in_channels, dw_channels, 3, stride=1)
