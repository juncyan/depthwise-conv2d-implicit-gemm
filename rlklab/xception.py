...
class LKConv(Reparams):
    def __init__(self, in_channels, small_kernels=5, dilation = 3, bias_attr=None):
        ...
        self.conv2 = nn.Conv2D(in_channels,in_channels,3,padding=4,dilation=4,groups=in_channels, bias_attr=bias_attr)
        # self.conv3 = nn.Conv2D(in_channels,in_channels,3,padding=8,dilation=8,groups=in_channels, bias_attr=bias_attr)
...
class LKConvBNLayer(nn.Layer):
    def __init__(self,
                 ...
                 act=None,
                 name=None):
        super(LKConvBNLayer, self).__init__()

        self._conv = LKConv(
            in_channels=input_channels,
            bias_attr=False)
        self._lbn = nn.BatchNorm2D(input_channels)
        self.gelu = nn.GELU()

        self._conv = nn.Conv2D(input_channels, output_channels, 3, 1, 1,bias_attr=False)
        self._bn = layers.SyncBatchNorm(
            num_features=output_channels, epsilon=1e-3, momentum=0.99)
...
class Seperate_LKConv(nn.Layer):
    def __init__(self,
                 input_channels,
                 output_channels,
                 stride,
                 ...
                 act=None,
                 name=None):
        super(Seperate_LKConv, self).__init__()

        self._conv1 = LKConv(input_channels,5,3)
        self._bn1 = layers.SyncBatchNorm(
            input_channels, epsilon=1e-3, momentum=0.99)
...
class Xception_LKBlock(nn.Layer):
    def __init__(self,
                 input_channels,
                 output_channels,
                 strides=1,
                 ...
                 activation_fn_in_separable_conv=False,
                 name=None):
        super(Xception_LKBlock, self).__init__()
