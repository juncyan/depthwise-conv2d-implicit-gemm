...
static bool is_contiguous(const paddle::Tensor& tensor) {
...
paddle::Tensor backward_filter_fp32(
    paddle::Tensor diff,
    paddle::Tensor input,
    paddle::Tensor weight)
{
    ...
    auto output = paddle::empty_like(weight);
    ...
    cutlass::TensorRef<ElementSrc, LayoutSrc> d_src((ElementSrc*)(input.data<ElementSrc>()),
        LayoutSrc().packed(options.input_size));
    cutlass::TensorRef<ElementDst, LayoutDst> d_diff((ElementDst*)(diff.data<ElementDst>()),
        LayoutDst().packed(options.output_size()));
    cutlass::TensorRef<typename Convolution::ElementGrad,
                       typename Convolution::LayoutGrad> d_filter((ElementFilter*)(output.data<ElementFilter>()),
